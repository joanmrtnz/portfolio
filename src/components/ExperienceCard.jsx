...
export default function ExperienceCard({ experience }) {
    ...
    return (
        <div ref={ref} className={`relative mb-[3rem] border-b border-[var(--section-outline)] pb-[3rem] last:mb-0 last:border-b-0 last:pb-0 transition-opacity duration-700 delay-300 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"}`}>
            ...
            <ol className={`mt-6 space-y-4 text-md text-[var(--slate)] relative pl-5 transition-opacity duration-700 delay-300 before:absolute before:left-[1.3rem] before:top-2 before:h-[calc(100%-0.5rem)] before:w-[2px] before:bg-[var(--dark-slate)] ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"}`}>
